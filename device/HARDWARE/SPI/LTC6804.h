...
*/
#define MD_FAST 1
#define MD_NORMAL 2//*******¿¼ÂÇ¼õÈ¥²Î¿¼µçÑ¹
...
void set_cs(uint8_t b);//Éè¶¨Ö¸¶¨Î»ºÅµÄ6804

void LTC6804_initialize(void);

void set_adc(uint8_t MD,uint8_t DCP,uint8_t CH,uint8_t CHG);//ÉèÖÃADC×´Ì¬

void LTC6804_adcv(uint8_t chip); //Æô¶¯µç³ØµçÑ¹ADC×ª»»ºÍÂÖÑ¯×´Ì¬

void LTC6804_adax(uint8_t chip);//Æô¶¯GPIO ADC×ª»»ºÍÂÖÑ¯×´Ì¬

uint8_t LTC6804_rdcv(uint8_t chip, uint8_t reg, uint8_t total_ic, uint16_t cell_codes[][12]);//¶ÁÈ¡µç³ØµçÑ¹

void LTC6804_rdcv_reg(uint8_t chip, uint8_t reg, uint8_t nIC, uint8_t *data);

int8_t LTC6804_rdaux(uint8_t chip, uint8_t reg, uint8_t nIC, uint16_t aux_codes[][6]);//¶ÁÈ¡¸¨Öú¼Ä´æÆ÷

void LTC6804_rdaux_reg(uint8_t chip, uint8_t reg, uint8_t nIC,uint8_t *data);

void LTC6804_clrcell(uint8_t chip); //Çå³ýµç³ØµçÑ¹¼Ä´æÆ÷A¡¢B¡¢C ºÍD¡£

void LTC6804_clraux(uint8_t chip);//Çå³ý¸¨Öú¼Ä´æÆ÷ A ºÍ B

void LTC6804_wrcfg(uint8_t chip, uint8_t nIC,uint8_t config[][6]);//Ð´ÅäÖÃ¼Ä´æÆ÷

int8_t LTC6804_rdcfg(uint8_t chip, uint8_t nIC, uint8_t r_config[][8]);//¶ÁÅäÖÃ¼Ä´æÆ÷

void delayMicroseconds(unsigned int x);

void wakeup_idle(uint8_t chip);//À¨ºÅºóÐè¼Óvoid£¬·ñÔò±¨¾¯
...
void LTC6804_cvst(uint8_t chip);//×Ô²âÊÔÄ£Ê½

void LTC6804_ADSTAT(uint8_t chip);//Æô¶¯×´Ì¬×éADC×ª»»£¬ÂË²¨Ä£Ê½

int8_t LTC6804_rdstat(uint8_t chip, uint8_t reg,uint8_t total_ic,uint16_t stat_codes[][6]);//¶Á×´Ì¬¼Ä´æÆ÷

void LTC6804_rdstat_reg(uint8_t chip, uint8_t reg,uint8_t total_ic,uint8_t *data);

void LTC6804_adcvax(uint8_t chip); //Æô¶¯×éºÏµç³ØµçÑ¹ÒÔ¼°GPIO1¡¢GPIO2×ª»»ºÍÂÖÑ¯×´Ì¬
#endif
