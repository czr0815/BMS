...
void SDCard_Init(){
	...
	//sd卡写入功能示例
	f_setlabel((const TCHAR*)"0:BMSCARD");   //修改SD卡名字，参数：磁盘号+名字，最长11个字符
	f_mkdir((const TCHAR*)"0:/SOC");   //在盘符0（SD卡默认挂载盘符)创建目录
}

void SDCard_Write_SOC(float soc){
	uint8_t *str;
	f_opendir(&dir, (const TCHAR*)"SOC");   //打开目录
	f_open(file, (const TCHAR*)"0:/SOC/soc.txt", FA_OPEN_ALWAYS|FA_READ|FA_WRITE);   //打开路径下的文件,开始使用内存中预先分配的缓存区；最后一个参数为打开模式
  ...
  */
	...
	str = (uint8_t*)mymalloc(SRAMIN, Str_SOC_Size);
	memset(str, 0, Str_SOC_Size);
	sprintf(str, "value:%f\r\n", soc);
	f_write(file, (u8*)str, strlen((const char*)str), &bw);    //向当前打开的文件写入内容，修改第二个和第三个参数即可；一次性不要写入太多，缓存大小有限制，写完一批f_close()先保存
	f_close(file);   //关闭文件，将缓存区中的内容写入文件  所以打开文件后务必记得关闭，否则创建或写入操作无效
	myfree(SRAMIN, str);
}

void SDCard_Write_SingleVoltage(float voltage, uint8_t id){
  uint8_t *str;
	f_opendir(&dir, (const TCHAR*)"SingleVoltage");   //打开目录
	f_open(file, (const TCHAR*)"0:/SingleVoltage/single_voltage.txt", FA_OPEN_ALWAYS|FA_READ|FA_WRITE);   //打开路径下的文件,开始使用内存中预先分配的缓存区；最后一个参数为打开模式
	f_lseek(file, f_size(file));

  str = (uint8_t*)mymalloc(SRAMIN, Str_SingleVoltage_Size);
	memset(str, 0, Str_SingleVoltage_Size);
	sprintf(str, "value:%f id:%d\r\n", voltage, id);
	f_write(file, (u8*)str, strlen((const char*)str), &bw);    //向当前打开的文件写入内容，修改第二个和第三个参数即可；一次性不要写入太多，缓存大小有限制，写完一批f_close()先保存
	f_close(file);   //关闭文件，将缓存区中的内容写入文件  所以打开文件后务必记得关闭，否则创建或写入操作无效
	myfree(SRAMIN, str);
}

void SDCard_Write_Temperature(float temperature, uint8_t id){
  uint8_t *str;
	f_opendir(&dir, (const TCHAR*)"Temperature");   //打开目录
	f_open(file, (const TCHAR*)"0:/Temperature/temperature.txt", FA_OPEN_ALWAYS|FA_READ|FA_WRITE);   //打开路径下的文件,开始使用内存中预先分配的缓存区；最后一个参数为打开模式
	f_lseek(file, f_size(file));

  str = (uint8_t*)mymalloc(SRAMIN, Str_Temperature_Size);
	memset(str, 0, Str_Temperature_Size);
	sprintf(str, "value:%f id:%d\r\n", temperature, id);
	f_write(file, (u8*)str, strlen((const char*)str), &bw);    //向当前打开的文件写入内容，修改第二个和第三个参数即可；一次性不要写入太多，缓存大小有限制，写完一批f_close()先保存
	f_close(file);   //关闭文件，将缓存区中的内容写入文件  所以打开文件后务必记得关闭，否则创建或写入操作无效
	myfree(SRAMIN, str);
